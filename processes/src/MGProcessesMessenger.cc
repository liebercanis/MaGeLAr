...
theList( list )
{
  ...
  opticalOnlyCmd->AvailableForStates(G4State_PreInit);  
   ...
   fAngCorrCommand->AvailableForStates(G4State_PreInit);

   // /MG/process/storeICLevelData
...
void MGProcessesMessenger::SetNewValue( G4UIcommand *command, G4String newValue )
{
  ...
  else if( command == fStoreICLevelData )
    theList->SetStoreICLevelData(fStoreICLevelData->GetNewBoolValue(newValue));
