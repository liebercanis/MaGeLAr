 ...
 * 20-02-2019, KvSturm, "/MG/output/saveOnlyEventsWithEnergyDepositInGermanium"
 *                      save only if hits in detector (which can also be the dead volume)
 *
 */
//---------------------------------------------------------------------------//
  ...
  fOutputScheme(theOutput)
{
  ...
  // /MG/output/saveOnlyEventsWithEnergyDepositInGermanium
  fSaveOnlyEventsWithEnergyDepositInGeCmd = new G4UIcmdWithABool("/MG/output/saveOnlyEventsWithEnergyDepositInGermanium",this);
  fSaveOnlyEventsWithEnergyDepositInGeCmd->SetGuidance("save only events with energy deposit in detector (also deadlayer)");
  ...
  fSaveOnlyEventsWithEnergyDepositCmd->SetGuidance("save only events with energy deposit in detector active volume");
...
GEOutputGermaniumArrayMessenger::~GEOutputGermaniumArrayMessenger()
{
  ...
  delete fSaveOnlyEventsWithEnergyDepositInGeCmd;
...
void GEOutputGermaniumArrayMessenger::SetNewValue(G4UIcommand *command, G4String newValues)
{
  ...
  else if(command == fSaveOnlyEventsWithEnergyDepositInGeCmd)
    fOutputScheme->RequireHitInGe(fSaveOnlyEventsWithEnergyDepositInGeCmd->GetNewBoolValue(newValues));
  else if (command == fSaveOnlyEventsWithEnergyDepositCmd)
    fOutputScheme->RequireHit(fSaveOnlyEventsWithEnergyDepositCmd->GetNewBoolValue(newValues));
  else if (command == fSaveOnlyEventsWithCoincidenceCmd)
    fOutputScheme->RequireCoincidence(fSaveOnlyEventsWithCoincidenceCmd->GetNewBoolValue(newValues));
