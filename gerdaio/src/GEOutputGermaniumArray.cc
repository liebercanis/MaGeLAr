 ...
 *  12-02-2019, KvSturm, deleted boundary status check for optical photons. Starting from Geanat4.10.0 optical photons
 *                       trigger ProcessHits method automatically at the boundary 
 *   20-02-2019, KvSturm * add fFLAG_RequireHitInGe to write down events also if they just hit the detector dead layer
 *   12-03-2019, KvSturm * add more hits info to dl, pl and LAr
 *
 */
//---------------------------------------------------------------------------//
//
      ...
      fSphereRadius(30.0*cm)
{
  ...
  fFLAG_RequireHitInGe = false;  // hit in germanium
  fFLAG_RequireHit = false;      // hit in active germanium
...
void GEOutputGermaniumArray::DefineSchema()
{
  if(!SchemaDefined()) {
    ...
    if( !fReduceHitInfo1 ) {
      nT->Branch("hits_passivation_edep",hits_passivation_edep,"hits_passivation_edep[hits_passivation_totnum]/F");
      nT->Branch("hits_passivation_xpos",hits_passivation_xpos,"hits_passivation_xpos[hits_passivation_totnum]/F");
      nT->Branch("hits_passivation_ypos",hits_passivation_ypos,"hits_passivation_ypos[hits_passivation_totnum]/F");
      nT->Branch("hits_passivation_zpos",hits_passivation_zpos,"hits_passivation_zpos[hits_passivation_totnum]/F");
      nT->Branch("hits_passivation_iddet",hits_passivation_iddet,"hits_passivation_iddet[hits_passivation_totnum]/I");
      nT->Branch("hits_passivation_time",hits_passivation_time,"hits_passivation_time[hits_passivation_totnum]/F");
      nT->Branch("hits_passivation_trackid",hits_passivation_trackid,"hits_passivation_trackid[hits_passivation_totnum]/I");
      nT->Branch("hits_passivation_trackpdg",hits_passivation_trackpdg,"hits_passivation_trackpdg[hits_passivation_totnum]/I");
    }
    // hits in deadlayer
    ...
    if( !fReduceHitInfo1 ) {
      nT->Branch("hits_deadlayer_edep",hits_deadlayer_edep,"hits_deadlayer_edep[hits_deadlayer_totnum]/F");
      nT->Branch("hits_deadlayer_xpos",hits_deadlayer_xpos,"hits_deadlayer_xpos[hits_deadlayer_totnum]/F");
      nT->Branch("hits_deadlayer_ypos",hits_deadlayer_ypos,"hits_deadlayer_ypos[hits_deadlayer_totnum]/F");
      nT->Branch("hits_deadlayer_zpos",hits_deadlayer_zpos,"hits_deadlayer_zpos[hits_deadlayer_totnum]/F");
      nT->Branch("hits_deadlayer_iddet",hits_deadlayer_iddet,"hits_deadlayer_iddet[hits_deadlayer_totnum]/I");
      nT->Branch("hits_deadlayer_time",hits_deadlayer_time,"hits_deadlayer_time[hits_deadlayer_totnum]/F");
      nT->Branch("hits_deadlayer_trackid",hits_deadlayer_trackid,"hits_deadlayer_trackid[hits_deadlayer_totnum]/I");
      nT->Branch("hits_deadlayer_trackpdg",hits_deadlayer_trackpdg,"hits_deadlayer_trackpdg[hits_deadlayer_totnum]/I");
    }
    // hits in LAr
    nT->Branch("hits_LAr_totnum",&hits_LAr_totnum,"hits_LAr_totnum/I");
    nT->Branch("hits_LAr_tote",&hits_LAr_tote,"hits_LAr_tote/F");
    if( !fReduceHitInfo1 ) {
      nT->Branch("hits_LAr_edep",hits_LAr_edep,"hits_LAr_edep[hits_LAr_totnum]/F");
      nT->Branch("hits_LAr_xpos",hits_LAr_xpos,"hits_LAr_xpos[hits_LAr_totnum]/F");
      nT->Branch("hits_LAr_ypos",hits_LAr_ypos,"hits_LAr_ypos[hits_LAr_totnum]/F");
      nT->Branch("hits_LAr_zpos",hits_LAr_zpos,"hits_LAr_zpos[hits_LAr_totnum]/F");
      nT->Branch("hits_LAr_time",hits_LAr_time,"hits_LAr_time[hits_LAr_totnum]/F");
      nT->Branch("hits_LAr_trackid",hits_LAr_trackid,"hits_LAr_trackid[hits_LAr_totnum]/I");
      nT->Branch("hits_LAr_trackpdg",hits_LAr_trackpdg,"hits_LAr_trackpdg[hits_LAr_totnum]/I");
    }
    ...
    // energy deposit in water and LAr
    nT->Branch("ene_in_water",&hits_water_tote,"ene_in_water/F");
    nT->Branch("ene_in_nitrogen",&hits_LAr_tote,"ene_in_nitrogen/F");

    // energy deposit in plastic scintillator
    nT->Branch("ene_in_scint",&hits_scintillator_tote,"ene_in_scint/F");
...
void GEOutputGermaniumArray::FillHitsToNtupleTrees(const G4Event *evt){
  ...
  if (PCHC) {
    ...
    for (G4int i=0; i<hits_passivation_totnum; i++) {
      ...
      hits_passivation_iddet[i]    = (*PCHC)[i]->GetCopynumber();
      hits_passivation_time[i]     = G4ToRoot((*PCHC)[i]->GetHittime()/ns);
      hits_passivation_trackid[i]  = (*PCHC)[i]->GetTrackID();
      hits_passivation_trackpdg[i] = (*PCHC)[i]->GetTrackPDG();
    }
  }
  ...
  if (DCHC) {
    ...
    for (G4int i=0; i<hits_deadlayer_totnum; i++) {
      ...
      hits_deadlayer_iddet[i]    = (*DCHC)[i]->GetCopynumber();
      hits_deadlayer_time[i]     = G4ToRoot((*DCHC)[i]->GetHittime()/ns);
      hits_deadlayer_trackid[i]  = (*DCHC)[i]->GetTrackID();
      hits_deadlayer_trackpdg[i] = (*DCHC)[i]->GetTrackPDG();
    }
  }
  ...
  if (NCHC) {
    ...
    for (G4int i=0; i<hits_LAr_totnum; i++)
    {
      ...
      hits_LAr_time[i]     = G4ToRoot((*NCHC)[i]->GetHittime()/ns);
      hits_LAr_trackid[i]  = (*NCHC)[i]->GetTrackID();
      hits_LAr_trackpdg[i] = (*NCHC)[i]->GetTrackPDG();
    }
  }

  //Energy deposit in Water
  if (WCHC) {
    G4int hits_water_totnum = WCHC->entries();
    
    for (G4int i=0;i<hits_water_totnum; i++) {
      hits_water_tote += G4ToRoot( (*WCHC)[i]->GetEdep() / GeV );
    }
  }

  //Energy deposit in Plastic Scintillator
  if (PSHC) {
    G4int hits_scintillator_totnum = PSHC->entries();
    
    for (G4int i=0; i<hits_scintillator_totnum; i++)
    {
      hits_scintillator_tote += G4ToRoot( (*PSHC)[i]->GetEdep() / MeV );
    }
  }
  ...
  MGLog(debugging) << "Energy in germanium " << hits_tote       << " keV" << endlog;
  MGLog(debugging) << "Energy in water "     << hits_water_tote << " GeV" << endlog;
  MGLog(debugging) << "Energy in LAr "       << hits_LAr_tote   << " MeV" << endlog;
  MGLog(debugging) << "Energy in plastic scintillator " << hits_scintillator_tote << " MeV" << endlog;
  if (StoreLArInstrumentationHits) {
...
{
  if (IsMother()) {
    if(fFLAG_RequireHitInGe) {
      if(hits_deadlayer_tote>0.0 || hits_passivation_tote>0.0 || hits_tote>0.0) FillTree();
    }
    else if(fFLAG_RequireHit) {
      if(hits_tote>0.0) FillTree();
    }
    else if(fFLAG_RequireCoincidence) {
      if(det_totnum>1) FillTree();
    }
    else FillTree();
  }
}
...
void GEOutputGermaniumArray::RootSteppingAction(const G4Step *step)
{
    ...
    if (particleType == "nucleus" && step->GetTrack()->GetParentID()>0) {
      ...
      if (energy < 0.1*keV) {
        ...
        if (lifetime > fKillLifetimeLongLivedNuclei) {//kill long-lived nuclei
          ...
          MGLog(debugging) << "Killing " << ion->GetParticleName() << 
            " (lifetime = " << lifetime/s << "s) " << endlog;
  ...
  if (StoreLArInstrumentationHits) {
    ...
    if (LArInst_design == LArInstr_fibers || LArInst_design == LArInstr_PMT || LArInst_design == LArInstr_hybrid || 
        LArInst_design == LArInstr_APD || LArInst_design == LArInstr_hybridAPD) {
        // Get the track
