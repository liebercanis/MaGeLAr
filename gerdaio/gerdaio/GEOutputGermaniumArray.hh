 ...
 *                         change old ene_in_water/scint variables to new style hits_water/scintillator_tote
 *
 */
...
//{MGLog(routine)<<"now processing tracks in waiting stack"<<endlog;};
  ...
  // write reduced information in the output
  void RequireHitInGe    (G4bool flag) { fFLAG_RequireHitInGe     = flag; };
  void RequireHit        (G4bool flag) { fFLAG_RequireHit         = flag; };
  void RequireCoincidence(G4bool flag) { fFLAG_RequireCoincidence = flag; };
...
private:
  ...
  Float_t hits_water_tote;
  Float_t hits_scintillator_tote;

  Int_t neutronflag; ///< neutron flag

  // require hit in germanium
  Int_t fFLAG_RequireHitInGe;
  // require hit in active germanium
  ...
  Int_t    hits_passivation_iddet[MAX_NHITS];  // which Ge detector this hit is in
  Float_t  hits_passivation_time[MAX_NHITS];
  Int_t    hits_passivation_trackid[MAX_NHITS]; // which track (in trajectory group)
  Int_t    hits_passivation_trackpdg[MAX_NHITS]; // track identity
  ...
  Int_t    hits_deadlayer_iddet[MAX_NHITS];  // which Ge detector this hit is in
  Float_t  hits_deadlayer_time[MAX_NHITS];
  Int_t    hits_deadlayer_trackid[MAX_NHITS]; // which track (in trajectory group)
  Int_t    hits_deadlayer_trackpdg[MAX_NHITS]; // track identity
  ...
  Float_t  hits_LAr_time[MAX_NHITS];
  Int_t    hits_LAr_trackid[MAX_NHITS]; // which track (in trajectory group)
  Int_t    hits_LAr_trackpdg[MAX_NHITS]; // track identity
