 ...
 * 02-03-2019, Added messenger command to specify whether to partition
 *             Pb shield. M. Buuck
 */
//---------------------------------------------------------------------------//
//
...
#include "mjdemonstrator/MJDemoRadShieldPbMiddle.hh"
  ...
  MJVDemoAssembly(fName, serialNumber, "RadShield")
{
  ...
  if (fSerialNumber.contains("partition")) fIsPartitioned = true;
  if (fIsPartitioned) {
    fPbInnerPtr = new MJDemoRadShieldPbInner(fName+"_RadShieldPbInner_001", "867-5309"); 
    fPbMiddlePtr = new MJDemoRadShieldPbMiddle(fName+"_RadShieldPbMiddle_001", "867-5309"); 
    fPbOuterPtr = new MJDemoRadShieldPbOuter(fName+"_RadShieldPbOuter_001", "867-5309"); 
  }
  else fPbPtr = new MJDemoRadShieldPb(fName+"_RadShieldPb_001", "867-5309"); 
...
MJDemoRadShieldPb* MJDemoRadShieldAssembly::GetPb()
{
  if(fIsPartitioned) MGLog(warning) << "Pb shield is partitioned!" << endlog;
  return fPbPtr;
}

MJDemoRadShieldPbInner* MJDemoRadShieldAssembly::GetPbInner()
{
  if(!fIsPartitioned) MGLog(warning) << "Pb shield is not partitioned!"
                                     << endlog;
  return fPbInnerPtr;
}

MJDemoRadShieldPbMiddle* MJDemoRadShieldAssembly::GetPbMiddle()
{
  if(!fIsPartitioned) MGLog(warning) << "Pb shield is not partitioned!"
                                     << endlog;
  return fPbMiddlePtr;
...
MJDemoRadShieldPbOuter* MJDemoRadShieldAssembly::GetPbOuter()
{
  if(!fIsPartitioned) MGLog(warning) << "Pb shield is not partitioned!"
                                     << endlog;
  return fPbOuterPtr;
                ...
                G4RotationMatrix* assemRotation, G4LogicalVolume* motherLogical)
{
  ...
  *shieldCuOuteraffine*=*assemAffine;  
  ...
  if(fIsPartitioned) {
    G4ThreeVector *shieldPbInnerlocalPos = new G4ThreeVector(0, 0, 0);
    G4RotationMatrix *shieldPbInnerlocalRot = new G4RotationMatrix();
    G4AffineTransform *shieldPbInneraffine = new G4AffineTransform(shieldPbInnerlocalRot,*shieldPbInnerlocalPos);
    *shieldPbInneraffine*=*assemAffine;  
    G4ThreeVector *shieldPbInnerglobalPos = new G4ThreeVector(shieldPbInneraffine->NetTranslation());
    G4RotationMatrix *shieldPbInnerglobalRot= new G4RotationMatrix(shieldPbInneraffine->NetRotation());
    fPbInnerPtr->Place(shieldPbInnerglobalPos, shieldPbInnerglobalRot, motherLogical);
    delete shieldPbInnerlocalPos;
    delete shieldPbInnerlocalRot;
    delete shieldPbInneraffine;
    
    G4ThreeVector *shieldPbMiddlelocalPos = new G4ThreeVector(0, 0, 0);
    G4RotationMatrix *shieldPbMiddlelocalRot = new G4RotationMatrix();
    G4AffineTransform *shieldPbMiddleaffine = new G4AffineTransform(shieldPbMiddlelocalRot,*shieldPbMiddlelocalPos);
    *shieldPbMiddleaffine*=*assemAffine;  
    G4ThreeVector *shieldPbMiddleglobalPos = new G4ThreeVector(shieldPbMiddleaffine->NetTranslation());
    G4RotationMatrix *shieldPbMiddleglobalRot= new G4RotationMatrix(shieldPbMiddleaffine->NetRotation());
    fPbMiddlePtr->Place(shieldPbMiddleglobalPos, shieldPbMiddleglobalRot, motherLogical);
    delete shieldPbMiddlelocalPos;
    delete shieldPbMiddlelocalRot;
    delete shieldPbMiddleaffine;
    
    G4ThreeVector *shieldPbOuterlocalPos = new G4ThreeVector(0, 0, 0);
    G4RotationMatrix *shieldPbOuterlocalRot = new G4RotationMatrix();
    G4AffineTransform *shieldPbOuteraffine = new G4AffineTransform(shieldPbOuterlocalRot,*shieldPbOuterlocalPos);
    *shieldPbOuteraffine*=*assemAffine;  
    G4ThreeVector *shieldPbOuterglobalPos = new G4ThreeVector(shieldPbOuteraffine->NetTranslation());
    G4RotationMatrix *shieldPbOuterglobalRot= new G4RotationMatrix(shieldPbOuteraffine->NetRotation());
    fPbOuterPtr->Place(shieldPbOuterglobalPos, shieldPbOuterglobalRot, motherLogical);
    delete shieldPbOuterlocalPos;
    delete shieldPbOuterlocalRot;
    delete shieldPbOuteraffine;
  }
    ...
    *shieldPbaffine*=*assemAffine;  
    ...
    fPbPtr->Place(shieldPbglobalPos, shieldPbglobalRot, motherLogical);
