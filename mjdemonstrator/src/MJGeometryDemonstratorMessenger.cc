...
MJGeometryDemonstrator *demo) : fDemonstrator(demo)
{
  ...
  // /MG/demonstrator/partitionPbShield
  fPartitionPbShieldCmd = new G4UIcmdWithABool("/MG/demonstrator/partitionPbShield", this);
  fPartitionPbShieldCmd->SetGuidance("Set to true if you want to partition the Pb shield into the innermost 5cm, 5cm-15cm, and the outer part of the shield.");
  fPartitionPbShieldCmd->SetDefaultValue(true);
  fPartitionPbShieldCmd->AvailableForStates(G4State_PreInit);
...
void MJGeometryDemonstratorMessenger::SetNewValue(G4UIcommand *cmd, G4String nV)
{
  ...
  else if(cmd == fPartitionPbShieldCmd)
    fDemonstrator->SetPbShieldPartition(fPartitionPbShieldCmd->GetNewBoolValue(nV));
