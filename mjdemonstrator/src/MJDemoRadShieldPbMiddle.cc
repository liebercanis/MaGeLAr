...
//      
// CLASS IMPLEMENTATION:  MJDemoRadShieldPbMiddle.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
 ...
 * 
 * REVISION: Jan 31, 2019 mbuuck@uw.edu
 * Partitioned again by adding middle 10cm-thick layer
...
MJDemoRadShieldPbMiddle::MJDemoRadShieldPbMiddle(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "ShieldPbMiddleDwg", "Lead-Ain")
{;}

MJDemoRadShieldPbMiddle::MJDemoRadShieldPbMiddle(const MJDemoRadShieldPbMiddle & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoRadShieldPbMiddle::~MJDemoRadShieldPbMiddle()
{;}

G4LogicalVolume* MJDemoRadShieldPbMiddle::ConstructPart()
{
  ...
  if (pVol == NULL){
    G4Box* body = new G4Box("body", 20.001*25.4*mm+15.0*cm, 13.001*25.4*mm+15.0*cm, 14.001*25.4*mm+15.0*cm);
...
*/
    ...
    G4SubtractionSolid* leadShieldMiddle = new G4SubtractionSolid("leadShieldMiddle", body7, tubeCut, rotation2,
    ...
    pVol = new G4LogicalVolume(leadShieldMiddle, material, logicalName);
    pVol->SetVisAttributes(blackVisAtt); 
    MGLog(debugging) << "Created Lead Shield Middle Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing Lead Shield Middle Logical" << endlog; 
  return pVol;
}  
