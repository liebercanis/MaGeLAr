...
void MGGeneratorLGNDLiquidArgon::PositionDecider()
{
  ...
  if(fZ ==0)
    fZ = cryostat->GetInnerHeight();
  //can set fRadius in messenger
  if(fRadiusMax == 0) 
    fRadiusMax = cryostat->GetInnerRadius();
...
void MGGeneratorLGNDLiquidArgon::GeneratePrimaryVertex(G4Event *event)
{
  ...
  for(int k = 0; k < yBins;k++){
    for(int j = 0; j < xBins;j++){
      ...
      x = xMin + j*fBinWidth;
      /*
      if(( xMin + j*fBinWidth) < -fRadiusMin)
          x = xMin + j*fBinWidth;
      else
          x = xMin + j*fBinWidth + 2*fRadiusMin;
      */

      G4double y;
      y = yMin + k*fBinWidth;
      /*
      if(( yMin + k*fBinWidth) < -fRadiusMin)
          y = yMin + k*fBinWidth;
      else
          y = yMin + k*fBinWidth + 2*fRadiusMin;
      */
      if(sqrt(x*x+y*y) < fRadiusMin){
        //G4cout<<"Generating point randomly ("<<x<<","<<y<<","<<fZ<<") ... R "<<sqrt(x*x+y*y)<<G4endl;
        continue;
      }
