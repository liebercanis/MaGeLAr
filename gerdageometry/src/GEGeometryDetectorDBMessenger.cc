 ...
 * 12-09-2018, Added macro command to switch between dat and json geometry file format, KvSturm
...
GEGeometryDetectorDBMessenger::GEGeometryDetectorDBMessenger(GEGeometryDetectorDB *DetectorDB) : fDetectorDB(DetectorDB)
{
  ...
  fCrystalGeometryFileCmd = new G4UIcmdWithAString("/MG/geometry/detector/geometryfile", this);
  fCrystalGeometryFileCmd -> SetParameterName("GeometryFile", true);
  fCrystalGeometryFileCmd -> SetDefaultValue("./gerdageometry/files/geometry.json");

  // switch to keep compatibility with old geometry file format
  fCrystalGeometryFileFormatCmd = new G4UIcmdWithAString("/MG/geometry/detector/geometryfileformat", this);
  fCrystalGeometryFileFormatCmd -> SetParameterName("GeometryFileFormat", true);
  fCrystalGeometryFileFormatCmd -> SetDefaultValue("json");
  fCrystalGeometryFileFormatCmd -> SetCandidates("json dat");

  fCrystalMatrixFileCmd = new G4UIcmdWithAString("/MG/geometry/detector/matrixfile", this);
  fCrystalMatrixFileCmd -> SetParameterName("MatrixFile", true);
  fCrystalMatrixFileCmd -> SetDefaultValue("./gerdageometry/files/matrix-PhaseII-dec15.dat");

  // switch to keep compatibility with old matrix file format
  fCrystalMatrixFileFormatCmd = new G4UIcmdWithAString("/MG/geometry/detector/matrixfileformat", this);
  fCrystalMatrixFileFormatCmd -> SetParameterName("MatrixFileFormat", true);
  fCrystalMatrixFileFormatCmd -> SetDefaultValue("json");
  fCrystalMatrixFileFormatCmd -> SetCandidates("json dat");
  ...
  fLArInstNFiberSegmentsCmd = new G4UIcmdWithAnInteger("/MG/geometry/LArInst/NFiberSegments",this);
  fLArInstNFiberSegmentsCmd->SetParameterName("NumberFiberSegments",false);
  fLArInstNFiberSegmentsCmd->SetDefaultValue(90); // default 15 chans x 6 sipms = 90 segments
  fLArInstNFiberSegmentsCmd->SetGuidance("Sets the number of fiber segments to be created. Default value: 90.");  
...
GEGeometryDetectorDBMessenger::~GEGeometryDetectorDBMessenger()
{
  ...
  delete fCrystalGeometryFileFormatCmd;
  delete fCrystalMatrixFileCmd;
  delete fCrystalMatrixFileFormatCmd;
  ...
  delete fLArInstNFiberSegmentsCmd;
...
void GEGeometryDetectorDBMessenger::SetNewValue(G4UIcommand *cmd, G4String newValue)
{
  ...
  else if (cmd == fCrystalGeometryFileFormatCmd)
    {
      G4String fileformat = newValue;
      fDetectorDB -> SetCrystalGeometryFileFormat(fileformat);
    }
  ...
  else if (cmd == fCrystalMatrixFileFormatCmd)
    {
      G4String fileformat = newValue;
      fDetectorDB -> SetCrystalMatrixFileFormat(fileformat);
    }
  ...
  else if ( cmd == fLArInstNFiberSegmentsCmd ) {
	  G4int flag = fLArInstNFiberSegmentsCmd->GetNewIntValue(newValue);
	  fDetectorDB->SetLArInstHybridNFiberSegments(flag);
	  MGLog(debugging) << "Number of fiber shroud segments in LAr Instrumentation hybrid design set to " << newValue << endlog;
  }
