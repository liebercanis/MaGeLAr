 ...
 * 11-09-2018 - Implemented JSON parser for geometry files - Luigi Pertoldi (pertoldi@pd.infn.it)
 * 12-09-2018, Added fCrystalGeometryFileFormat variable to switch between dat and json geometry file, KvSturm
 * 13-09-2018 - Implemented JSON parser for matrix files - Luigi Pertoldi (pertoldi@pd.infn.it)
 * 13-09-2018, Added macro command to switch between dat and json matrix file format, KvSturm
 */
  ...
  fCopperLinesConnection(nrOfStrings)
{
  ...
  for (int i = 0; i < nrOfStrings; ++i ) {
      for (int j = 0; j < nrOfRows; ++j ) {
          fArrayDetectorIndZPositionMatrix[i][j] = -1;
          for (int k = 0; k < nrOfMultiplets; ++k ) {
              fArrayDetectorTypeMatrix[i][j][k] = -1;
          }
      }
  }
  fSegmentNumber = 0;
  ...
  fLArInstHybridNFiberSegments    = 90;
 ...
 // cables
  ...
  SetCrystalGeometryFilename("geometry.json");
  SetCrystalGeometryFileFormat("json");
  SetCrystalMatrixFilename("matrix-PhaseII-dec15.json");
  SetCrystalMatrixFileFormat("json");
    ...
    fMiniShroudHeightPhaseI = 500.0 * mm; // slightly bigger MS to accommodate the 4 pairs of BEGe's
     for( G4int ii=0; ii<9; ii++ ){
       ...
       MGLog(debugging) << "MS string  " << ii << "   Nb. of crystals   " << counter
    		   << "  Z_shroud: " << (counter != 0 ? Z_shroud/counter : Z_shroud)
...
void GEGeometryDetectorDB::OpenInputFiles()
{
  ...
  std::string dlmaterial;
  std::string passivationmaterial;

  // Recommended and default format .json
  if( GetCrystalGeometryFileFormat() == "json" ) {

      katrin::KTree config;
      try {
          katrin::KTreeFile(GetCrystalGeometryFilename()).Read(config);
      }
      catch (katrin::KException &e) {
          MGLog(error) << e.what() << endlog;
      }

      // read in geometries
      auto idlist = config["gerdageometry"].KeyList();

      // sort the detector ids
      std::sort(
          idlist.begin(),
          idlist.end(),
      ...
      for (auto& k : idlist) {
          ...
          //
          // Read the data from geometry.json
          //

          // handy alias
          auto& d = config["gerdageometry"][k];

          // Now, read the data to corresponding variables
          // the [] operator throws a runtime error if the key does not exists,
          // unless a default value is specified with Or()
          try {
              detectorid           = d["id"]                                                .As<int>();
              name                 = d["name"]                                              .As<std::string>();
              geometry             = d["geometry-type"]                                     .As<int>();
              outerradius          = d["radius-mm"]                                         .As<double>();
              height               = d["height-mm"]                                         .As<double>();
              dlinnerthickness     = d["dead-layer"]["inner-thickness-nm"] .Or(0)           .As<double>();
              dlouterthickness     = d["dead-layer"]["outer-thickness-nm"] .Or(0)           .As<double>();
              dltopthickness       = d["dead-layer"]["top-thickness-nm"]   .Or(0)           .As<double>();
              dlbottomthickness    = d["dead-layer"]["bottom-thickness-nm"].Or(0)           .As<double>();
              passivationthickness = d["passivation-layer"]["thickness-nm"].Or(0)           .As<double>();
              grooveinnerradius    = d["groove"]["inner-radius-mm"]        .Or(0)           .As<double>();
              grooveouterradius    = d["groove"]["outer-radius-mm"]        .Or(0)           .As<double>();
              grooveheight         = d["groove"]["depth-mm"]               .Or(0)           .As<double>();
              groovedl             = d["groove"]["dead-layer-thickness-nm"].Or(0)           .As<double>();
              bulkmaterial         = d["bulk-material"]                    .Or("EnrichedGe").As<std::string>();
              dlmaterial           = d["dead-layer"]["material"]           .Or("EnrichedGe").As<std::string>();
              passivationmaterial  = d["passivation-layer"]["material"]    .Or("EnrichedGe").As<std::string>();

              // look for different keywords if we are talking about BEGEs
              if (geometry == 3 or geometry == 4 or geometry == 5) {
                  innerradius  = 0;
                  boredepth    = d["cone"]["height-mm"] .Or(0)       .As<double>();
                  angle        = d["cone"]["radius-mm"] .Or(0)       .As<double>();
                  nphisegments = d["cone"]["position"]  .Or("top")   .As<std::string>() == "top" ? 0 : 1;
                  nzsegments   = d["groove"]["position"].Or("bottom").As<std::string>() == "bottom" ? 0 : 1;
              }
              // otherwise, coaxials
              else {
                  innerradius  = d["bore-hole"]["radius-mm"]                .Or(0).As<double>();
                  boredepth    = d["bore-hole"]["depth-mm"]                 .Or(0).As<double>();
                  nphisegments = d["segmentation"]["number-of-phi-segments"].Or(0).As<int>();
                  nzsegments   = d["segmentation"]["number-of-z-segments"]  .Or(0).As<int>();
                  angle        = d["segmentation"]["angle-deg"]             .Or(0).As<double>();
              }
            }
            catch (katrin::KException &e) {
              MGLog(error) << "Something went wrong while reading detector with ID " << k << endlog;
              MGLog(error) << "can be either a mispelled mandatory keyword or an invalid value" << endlog;
              abort();
            }
            ...
            detector -> SetName(G4String(name.c_str()));
            ...
            detector -> SetBulkMaterial(G4String(bulkmaterial.c_str()));
            detector -> SetDLMaterial(G4String(dlmaterial.c_str()));
            detector -> SetPassivationMaterial(G4String(passivationmaterial.c_str()));
  ...
  // Old .dat geometry file parser (deprecated)
  else if( GetCrystalGeometryFileFormat() == "dat" ) {
      ...
      MGLog(error) << "Geometry file format " << GetCrystalGeometryFileFormat() << " not supported." << endlog;
      MGLog(error) << "Please use a .json (recommended) or .dat (deprecated) file or implement a new format." << endlog;
      abort();
  }
  ...
  // Recommended and default format .json
  if( GetCrystalMatrixFileFormat() == "json" ) {

      katrin::KTree matrixConfig;
      try {
          katrin::KTreeFile(GetCrystalMatrixFilename()).Read(matrixConfig);
      }
      catch (katrin::KException &e) {
          MGLog(error) << e.what() << endl;
      }

      auto detIDs = matrixConfig["gerdamatrix"]["detector-ids"];
      auto barLengths = matrixConfig["gerdamatrix"]["detector-ids"];
      if (fFlagCrystalMatrixWithZPosFile) barLengths = matrixConfig["gerdamatrix"]["holder-bars-length-cm"];

      // check if the file format is ok
      bool format_ok = true;
      // check that the two matrices have the same dimensions (strings and slots occupied)
      if (fFlagCrystalMatrixWithZPosFile) {
          if (detIDs.Length() != barLengths.Length()) format_ok = false;

          for (int i = 0; i < (int)detIDs.Length(); ++i) {
              if (detIDs[i].Length() != barLengths[i].Length()) format_ok = false;
          }
      }
      if (!format_ok) {
          MGLog(error) << "The format of " << GetCrystalMatrixFilename() << " is not correct!" << endlog;
          MGLog(error) << "\"detector-ids\" and \"holder-bars-length-cm\" must be of the same dimensions!" << endlog;
          abort();
      }
      // check if dimensions are compatible with max values
      else {
          if (detIDs.Length() > nrOfStrings) {
              MGLog(error) << "The format of " << GetCrystalMatrixFilename() << " is not correct!" << endlog;
              MGLog(error) << "Expected max 19 strings but got " << detIDs.Length() << " instead." << endlog;
              abort();
          }
          for (int i = 0; i < (int)detIDs.Length(); ++i) {
              if (detIDs[i].Length() > nrOfRows ) {
                  MGLog(error) << "The format of " << GetCrystalMatrixFilename() << " is not correct!" << endlog;
                  MGLog(error) << "Expected max 7 slots in string " << i << " but got "
                               << detIDs[i].Length() << " instead." << endlog;
                  abort();
              }
              for (int j = 0; j < (int)detIDs[i].Length(); ++j) {
                  if (detIDs[i][j].Length() > nrOfMultiplets) {
                      MGLog(error) << "The format of " << GetCrystalMatrixFilename() << " is not correct!" << endlog;
                      MGLog(error) << "Expected max 2 detectors in slot " << j << " of string " << i
                                   << " but got " << detIDs[i].Length() << " instead." << endlog;
                      abort();
                  }
              }
          }
      ...
      // fill it with values found
      for (int i = 0; i < (int)detIDs.Length(); ++i ) {
          // get geometry of first detector
          auto geotype = GetDetectorParameters(detIDs[i][0][0])->GetGeometry();

          for (int j = 0; j < nrOfRows; ++j ) {
              for (int k = 0; k < nrOfMultiplets; ++k ) {
                  // default is -1, it means 'no detector there'
                  fArrayDetectorTypeMatrix[i][j][k] = detIDs[i][j][k].Or(-1).As<int>();

                  // check if detector exists
                  if (fArrayDetectorTypeMatrix[i][j][k] > -1) {
                      auto detPar = GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][k]);
                      if (!detPar) {
                          MGLog(error) << "detectorid " << fArrayDetectorTypeMatrix[i][j][k]
                                       << " not found in geometry file!" << endlog;
                          abort();
                      }
                      // check if all detectors in a string share the same geometry type (except for 5 and 11)
                      else if (detPar->GetGeometry() != geotype) {
                          if ((geotype != 5 and geotype != 11) or (detPar->GetGeometry() != 5 and detPar->GetGeometry() != 11)) {
                              MGLog(error) << "A not-allowed mixture of different geometry types has"
                                           << " been detected in string " << i << "!" << endlog;
                              abort();
                          }
                      }
                  }
              }
              // if pair, check that is composed by BEGEs
              if (fArrayDetectorTypeMatrix[i][j][0] > -1 and fArrayDetectorTypeMatrix[i][j][1] > -1) {
                  if (GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][0])->GetGeometry() < 4 or
                      GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][1])->GetGeometry() < 4 or
                      GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][0])->GetGeometry() > 5 or
                      GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][1])->GetGeometry() > 5)
                  {
                      MGLog(error) << "In string " << i << " in detector slot " << j
                                   << ": only BEGEs can be placed in pairs!" << endlog;
                      abort();
                  }
              }

              // read in also holder bar lengths, if asked so
              if (fFlagCrystalMatrixWithZPosFile) {
                  fArrayDetectorIndZPositionMatrix[i][j] = barLengths[i][j].Or(-1).As<double>();
              }
          }
      }

      // dump array configuration
      for (int i = 0; i < nrOfStrings; ++i) {
          std::string line = "String " + std::to_string(i) + " | ";
          for (int j = 0; j < nrOfRows; ++j) {
              for (int k = 0; k < nrOfMultiplets; ++k) {
                  if (fArrayDetectorTypeMatrix[i][j][k] > -1) {
                      line += std::to_string(fArrayDetectorTypeMatrix[i][j][k])
                           + "[" + GetDetectorParameters(fArrayDetectorTypeMatrix[i][j][k])->GetName() + "] ";
                  }
              }
          }
          if (fArrayDetectorTypeMatrix[i][0][0] > -1) MGLog(trace) << line << endlog;
      }
  }

  // Old .dat matrix file parser (deprecated)
  else if( GetCrystalMatrixFileFormat() == "dat" ) {
          ...
          for (G4int i = 0; i < nrOfStrings; i++)
            {
              ...
              if (fFlagCrystalMatrixWithZPosFile) fInputFileMatrix >> readval >> indzposcrystal;
                  ...
                  if (readval.EndsWith(")")) //this is required
                    {
                      ...
                      MGLog(trace) << "Found detector pair at position (" << i << "," << j << ")" << endlog;
                  ...
                  for(unsigned int ii=0;ii<fDetectorParametersContainer.size();ii++)
                    {
                      GEGeometryDetectorParameters* detector = fDetectorParametersContainer[ii];
                      assert(detector);
                      if(detector->GetDetectorID()==fArrayDetectorTypeMatrix[i][j  ][0])
                          detector_Col_N = detector;
                      if(detector->GetDetectorID()==fArrayDetectorTypeMatrix[i][j-1][0])
                          detector_Col_Nminus1 = detector;
                    }
          ...
          if(fFlagCrystalMatrixWithZPosFile) for (G4int i = 0; i < nrOfStrings; i++)
            {
              if (fArrayDetectorIndZPositionMatrix[i][j]>=0 && fArrayDetectorTypeMatrix[i][j][0] <0)
                {
                  MGLog(error) << "    There is an individual z position without detector at Position (col="
                               << i << ", row=" << j << ")" << endlog;
                  abort();
                }
              if (fArrayDetectorIndZPositionMatrix[i][j]<0 && fArrayDetectorTypeMatrix[i][j][0] >=0)
                {
                  MGLog(error) << "    There is a detector at Position (col="
                               << i << ", row=" << j << ") without individual z position assigned." << endlog;
                  abort();
                }
            }
          // close file
    ...
    {
      MGLog(error) << "Matrix file format " << GetCrystalMatrixFileFormat() << " not supported." << endlog;
      MGLog(error) << "Please use a .json (recommended) or .dat (deprecated) file or implement a new format." << endlog;
      abort();
    }
