 ...
 *   -  -2017, Christoph, changed to segmented fiber shroud and "real" fiber coverage 
 */
//---------------------------------------------------------------------------//
...
//#include "G4PhysicalVolumeStore.hh" //->used for debugging
  ...
  fShroudPhysical_bottom(0),fShroudLogical(0)
{
  ...
  // Set to number of SiPMs (15 chans a 6 SiPMs)
  fNoOfSegments = fDetectorDB->GetLArInstHybridNFiberSegments();
...
//                            fTeflonPlateLogical_top,fTefOptSurf);
  ...
  G4Material *fMaterialTopPMTGlass = G4Material::GetMaterial("QuartzTopPMT"); //top PMT is different JJ
...
//                                               0.0036,0.0006,0.0001,0.0000};
  ...
  G4double PMTGlassEnergy[num_entries] = {
    1.61018*eV, 1.65312*eV, 1.69841*eV, 1.77120*eV, 1.87855*eV, 1.96800*eV,
    2.06640*eV, 2.13766*eV, 2.25426*eV, 2.38431*eV, 2.47968*eV, 2.58300*eV,
    2.69531*eV, 2.81782*eV, 2.95200*eV, 3.09960*eV, 3.26274*eV, 3.44401*eV,
    3.64659*eV, 3.87451*eV, 4.13281*eV, 4.59201*eV, 4.95937*eV, 5.39062*eV,
    6.19921*eV, 6.80111*eV, 7.29319*eV, 7.71046*eV, 10.7812*eV
  };

  // a little reflectivity is actually more realistic than zero
  ...
  if (LAr_TopPMT_scaling != 1) {
    MGLog(routine) << "Top PMT efficiency scaled down by: " << LAr_TopPMT_scaling << endlog;
  }

  G4double PMTGlassEfficiency[num_entries] = {
    0.0000, 0.0002, 0.0007, 0.0026, 0.0099, 0.0198, 0.0330,
    0.0447, 0.0727, 0.1131, 0.1352, 0.1513, 0.1749, 0.1985,
    0.2147, 0.2275, 0.2345, 0.2310, 0.2332, 0.2297, 0.2125,
    0.1730, 0.1536, 0.1312, 0.1191, 0.0000, 0.0000, 0.0000,
    0.0000
  };

  auto& S = LAr_TopPMT_scaling;
  G4double PMTGlassEfficiencyTop[num_entries] = {
    0.0000*S, 0.0002*S, 0.0007*S, 0.0026*S, 0.0099*S, 0.0198*S, 0.0330*S,
    0.0447*S, 0.0727*S, 0.1131*S, 0.1352*S, 0.1513*S, 0.1749*S, 0.1985*S,
    0.2147*S, 0.2275*S, 0.2345*S, 0.2310*S, 0.2332*S, 0.2297*S, 0.2125*S,
    0.1730*S, 0.1536*S, 0.1312*S, 0.1191*S, 0.0000*S, 0.0000*S, 0.0000*S,
    0.0000*S
  };
  ...
  topPMTGlassOptTable->AddProperty("EFFICIENCY",PMTGlassEnergy,PMTGlassEfficiencyTop,num_entries);
  fMaterialTopPMTGlass->SetMaterialPropertiesTable(topPMTGlassOptTable);

  // DO NOT change unless you know what you are doing:
  // surface is modeled as dielectric_metal because we are not interested what 
  // the photons do inside the glass we absorb them at the boundary.
  ...
  G4OpticalSurface* topPMTGlassOptSurface = new G4OpticalSurface("LArInstrPMTGlassTopSurface",glisur,polished,dielectric_metal);
...
void GEGeometryLArInstHybrid::ConstructFiberSubDesign() {
  ...
  G4double SiPMRindex[npoints_eff];

  G4double LAr_Fiber_coverage = fDetectorDB->GetLArInstFiberCoverage();
  MGLog(routine) << "Fiber coverage: " << LAr_Fiber_coverage << endlog;

  //fiber efficiency scaling relative to PMTs (only for monolitic implementation)
  G4double SiPM_bonding_scale = 0.7 * fEnableSegmentedFiberShroud ? 1. : LAr_Fiber_coverage;
  ...
  if (! fEnableSegmentedFiberShroud) {
    ...
    // reduced coverage = increased space between segments
    G4double  segmentSpacing = 2*radius*M_PI/fNoOfSegments*(1.-LAr_Fiber_coverage);
    MGLog(debugging) << "Segment spacing " << segmentSpacing/mm << " mm" << endlog;

    // - wls
    G4double half_angle = M_PI/fNoOfSegments-segmentSpacing/2./radius;
    G4VSolid* wlsSegment_solid = new G4Tubs("LArInstr_WLSSegment_Solid",
      radius-totalThickness/2.,radius+totalThickness/2.,height/2.,-half_angle,2*half_angle);
    // - Cladding
    half_angle -= fWLSThickness/radius;
    G4VSolid* claddingSegment_solid = new G4Tubs("LArInstr_CladdingSegment_Solid",
      radius-fFiberThickness/2.,radius+fFiberThickness/2.,height/2.,-half_angle,2*half_angle);
    // - fiber ends
    G4VSolid* endSegment_solid = new G4Tubs("LArInstr_EndSegment_Solid",
      radius-fFiberThickness/2.,radius+fFiberThickness/2.,fiber_end_height/2.,-half_angle,2*half_angle);
    G4LogicalVolume* endSegment_logvol_sensitive = new G4LogicalVolume(endSegment_solid,
      G4Material::GetMaterial("MetalSilicon_SiPM"),"LArInstr_EndSegment_Top_Logical");
    G4LogicalVolume* endSegment_logvol_reflective = new G4LogicalVolume(endSegment_solid,
      G4Material::GetMaterial("MetalSilicon"),"LArInstr_EndSegment_Bottom_Logical");
    // - fiber
    half_angle -= fFiberCladThickness/radius;
    G4VSolid* fiberSegment_solid = new G4Tubs("LArInstr_FiberSegment_Solid",
      radius-fFiberThickness/2.+fFiberCladThickness,radius+fFiberThickness/2.-fFiberCladThickness,height/2.,-half_angle,2*half_angle);
    ...
    for(int i=0;i<fNoOfSegments;i++) {
      ...
      //place wls
      G4LogicalVolume* wlsSegment_logvol = new G4LogicalVolume(wlsSegment_solid,
        fTPB,Form("LArInstr_WLSOuterSegment_Logical_%d",i));
      wlsSegment_logvol->SetVisAttributes(wls_vis_att);
      G4VPhysicalVolume* wlsSegment_Physical = new G4PVPlacement(z_rot,
                        G4ThreeVector(fDetCenter.x(),fDetCenter.y(),fDetCenter.z()),
                        wlsSegment_logvol,
                        Form("LArInstr_WLSSegment_Physical_%d",i),
                        fMotherLogical,
                        false,
                        i);
      // consider the surface rough from both sides
      new G4LogicalBorderSurface(Form("LArInstr_WLSOuter_Physical_surf_in_%d",i),
                                 fMotherPhysical,wlsSegment_Physical,fWLSoptSurf);
      new G4LogicalBorderSurface(Form("LArInstr_WLSOuter_Physical_surf_out_%d",i),
                                 wlsSegment_Physical,fMotherPhysical,fWLSoptSurf);

      //place cladding
      G4LogicalVolume* claddingSegment_logvol = new G4LogicalVolume(claddingSegment_solid,
        fiber_cladding_material,Form("LArInstr_CladdingSegment_Logical_%d",i));
      ...
      G4VPhysicalVolume* claddingSegment_Physical = new G4PVPlacement(0,
                        ...
                        Form("LArInstr_CladdingSegment_Physical_%d",i),
                        wlsSegment_logvol,
                        false,
                        i);

      //place fiber
      G4LogicalVolume* fiberSegment_logvol = new G4LogicalVolume(fiberSegment_solid,
        fiber_material,Form("LArInstr_FiberSegment_Logical_%d",i));
      fiberSegment_logvol->SetVisAttributes(fiber_vis_att);
       G4VPhysicalVolume* fiberSegment_Physical = new G4PVPlacement(0,
                        G4ThreeVector(0.0,0.0,0.0),
                        fiberSegment_logvol,
                        Form("LArInstr_FiberSegment_Physical_%d",i),
                        claddingSegment_logvol,
                        false,
                        i);
      ...
      G4VPhysicalVolume* sensSegment_Physical = new G4PVPlacement(z_rot,
                        ...
                        Form("LArInstr_FiberOuter_SensitiveEnd_Physical_%d",i),
                        fMotherLogical,
                        false,
                        i);
      // attach the sensitive surfaces
      new G4LogicalBorderSurface(Form("LArInstr_CladdingSegment_Sensitive_surf_%d",i),
                                 claddingSegment_Physical,
                                 sensSegment_Physical,
                                 sipmOptSurface);
      new G4LogicalBorderSurface(Form("LArInstr_FiberSegment_Sensitive_surf_%d",i),
      ...
      G4VPhysicalVolume *reflSegment_Physical = new G4PVPlacement(z_rot,
                        ...
                        Form("LArInstr_ReflectiveEndSegment_Physical_%d",i),
                        fMotherLogical,
                        false,
                        i);
      // attach the reflective surfaces
      new G4LogicalBorderSurface(Form("LArInstr_CladdingSegment_Reflective_surf_%d",i),
                                 claddingSegment_Physical,reflSegment_Physical,fiberReflOptSurface);
      new G4LogicalBorderSurface(Form("LArInstr_FiberSegment_Reflective_surf_%d",i),
                                 fiberSegment_Physical,reflSegment_Physical,fiberReflOptSurface);
    }
  ...
  //for(auto &kv : *G4PhysicalVolumeStore::GetInstance())
  //  MGLog(debugging) << kv->GetName() << ", " << kv->GetCopyNo() << endlog;
