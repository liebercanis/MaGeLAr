...
G4LogicalVolume* LGND_JSONDet::BuildLogicalVolume()
{
  ...
  G4String logicalName = fSerialNumber + "_" + fPartMaterial;
  ...
  if (pVol == NULL){
    ...
    MGLog(debugging) << "Created " << fSerialNumber << " Detector Logical" << endlog;

    delete[] r_i;
    delete[] r_o;
    delete[] z;
  }
  else  MGLog(debugging) << "Using pre-existing " << fSerialNumber << " Detector Logical" << endlog;
  return pVol;
}
