...
LGND_FiberWLSCoatingCurved::LGND_FiberWLSCoatingCurved(G4String partName, G4String serialNumber,G4double length) :
  LGND_Part(partName, serialNumber, "LGND_FiberWLSCoatingCurved", "TPBOnNylon"),
  fLength(length*mm)
{;}

LGND_FiberWLSCoatingCurved::LGND_FiberWLSCoatingCurved(G4String partName, G4String serialNumber,G4double length,G4String shape,G4double curvatureAngle) :
  LGND_Part(partName, serialNumber, "LGND_FiberWLSCoatingCurved", "TPBOnNylon"),
  fLength(length*mm),
  fShape(shape),
...
LGND_FiberWLSCoatingCurved::LGND_FiberWLSCoatingCurved(G4String partName, G4String serialNumber) :
  LGND_Part(partName, serialNumber, "LGND_FiberWLSCoatingCurved", "TPBOnNylon")
{;}

LGND_FiberWLSCoatingCurved::LGND_FiberWLSCoatingCurved(const LGND_FiberWLSCoatingCurved & rhs) :
  LGND_Part(rhs)
{;}

LGND_FiberWLSCoatingCurved::~LGND_FiberWLSCoatingCurved()
{;}

G4LogicalVolume* LGND_FiberWLSCoatingCurved::BuildLogicalVolume()
{
  G4LogicalVolumeStore *storePtr = G4LogicalVolumeStore::GetInstance();
  G4String logicalName = fDrawingNumber + "_" + fPartMaterial+"x"+G4UIcommand::ConvertToString(fLength);
  G4LogicalVolume* pVol = storePtr->GetVolume(logicalName, false);
  G4VSolid* wlsSolid;
  G4double lengthInner = .5*mm;
  G4double lengthOuter = .5*mm + 600.*nm;
  fRadius = lengthOuter-lengthInner;
  ...
  if(pVol == NULL){
    ...
    if(fShape == "square"){
      ...
      */
      G4VSolid* innerSolid = new G4Tubs("innerSolidCurved",fLength-lengthInner,fLength+lengthInner,lengthInner,0,fCurvatureAngle);//0.5*pi);
      G4VSolid* outerSolid = new G4Tubs("outerSolidCurved",fLength-lengthOuter,fLength+lengthOuter,lengthOuter,0,fCurvatureAngle);//0.5*pi);
      wlsSolid = new G4SubtractionSolid("wlsSolid",outerSolid,innerSolid);
