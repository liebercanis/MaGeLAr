...
dnl    MGDO_TABREE_LIB_DIR       Where the MGDO Tabree library is
...
dnl    MGDO_TABREE_LIB           Name of MGDO Tabree lib
...
dnl    MGDO_TABREE_INC_DIR       Name of MGDO Tabree include directory
dnl
...
[
  ...
  if test ! x"$MGDOCONF" = "xno" ; then
    ...
    MGDO_LIBS=`$MGDOCONF --libs --root --transforms --gerda --tabree`
    no_mgdo=""
    ...
    MGDO_TABREE_LIB_DIR="$MGDODIR/lib"
    ...
    if test -f "$MGDO_TABREE_LIB_DIR/lib$MGDO_TABREE_LIB.so"; then
      # .a file exists
      echo "MGDO Tabree lib found."
    elif test -f "$MGDO_TABREE_LIB_DIR/lib$MGDO_TABREE_LIB.dylib"; then
      # so file exists. 
      echo "MGDO Tabree lib found."
    elif test -f "$MGDO_TABREE_LIB_DIR/lib$MGDO_TABREE_LIB.a"; then
      echo "MGDO Tabree lib found."
    else
      # no lib files!
      echo "No MGDO Tabree lib file found!"
      no_mgdo="yes"
    fi
    ...
    MGDO_TABREE_INC_DIR="$MGDODIR/Tabree"
    ...
    echo "Using MGDO_TABREE_INC_DIR: $MGDO_TABREE_INC_DIR"
    ...
    MGDO_CFLAGS=["-I${MGDO_BASE_INC_DIR} -I${MGDO_ROOT_INC_DIR} -I${MGDO_TRANSFORMS_INC_DIR} -I${MGDO_GERDA_INC_DIR} -I${MGDO_TABREE_INC_DIR}"]
    MGDO_LIBS=["-L${MGDO_BASE_LIB_DIR} -l${MGDO_BASE_LIB} -l${MGDO_ROOT_LIB} -l${MGDO_TRANSFORMS_LIB} -l${MGDO_GERDA_LIB} -l${MGDO_TABREE_LIB}"]
